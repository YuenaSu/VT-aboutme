...
export const addBook = (bookWithoutId: any, history: RouteComponentProps['history']): Thunk<string, unknown> => (
  dispatch: Dispatch<FSA<string, unknown>>,
): void => {
  apiFetcher
    .post(apiConfig.endpoints.books.addBook, bookWithoutId) // Send the bookData as the request payload
    .then((res: Book) => {
      dispatch({ type: BOOKS_ACTIONS.BOOK_ADD, payload: res }); // Dispatch an action to add the book to the store
